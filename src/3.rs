...
fn main() {
    ...
    for line in input.iter() {
        x_coords.push(
            line[line.find("@").unwrap() + 2..line.find(",").unwrap()]
                .parse::<i32>()
                .unwrap(),
        );
        y_coords.push(
        ...
        x_sizes.push(
            line[line.find(":").unwrap() + 2..line.find("x").unwrap()]
                .parse::<i32>()
                .unwrap(),
        );
        y_sizes.push(line[line.find("x").unwrap() + 1..].parse::<i32>().unwrap());
        field_x_size =
            field_x_size.max(x_coords.last().copied().unwrap() + x_sizes.last().copied().unwrap());
        field_y_size =
            field_y_size.max(y_coords.last().copied().unwrap() + y_sizes.last().copied().unwrap());
    }
